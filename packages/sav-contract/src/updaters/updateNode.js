...
import {noticeString, ensureDir, outputFile, inputFile,
  readDir, pathExists, getBackRoutes} from '../utils/util.js'
...
function syncNodeAction (dir, modalName, modal) {
  ...
  return pathExists(file).then(async (exists) => {
    ...
    if (exists) {
      ...
      let data = createClassModule(modalName, methods)
